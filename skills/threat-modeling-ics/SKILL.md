...
- `<Device_Name>_Threat_Model_Generated.csv`
  ...
  0;Spoofing the MCU Process;Spoofing;<Device_Name>;Debugger to MCU over JTAG;Low;Not Applicable;;MCU may be spoofed by an attacker and this may lead to information disclosure by Debugger Probe. Consider using a standard authentication mechanism to identify the destination process.;"The MCU firmware is the target of the JTAG debug session, not a process with an identity to spoof. Evidence for Threat Actor classification is physical debug-port dependency AV:P, requirement for direct maintenance access, and absence of any remote network precondition. That profile matches Insider Threat as the minimum actor because trusted local access is required, while Thrill Seeker and Hacktivist paths are excluded due to no internet-exposed interface. Nation-State Actor is not required because the scenario does not depend on custom tooling, covert persistence, or strategic sabotage objectives.";Generated;T0843;TID-116, TID-119;CWE-1191;CVSS:4.0/AV:P/AC:L/AT:N/PR:N/UI:N/VC:N/VI:N/VA:N/SC:N/SI:N/SA:N;"0,0";None;Info;Info;Insider Threat;Avoidance;Not Required
  11;Spoofing the RS-485 Interface Process;Spoofing;<Device_Name>;PLC to RS-485 (Modbus RTU);Medium;Needs Investigation;;RS-485 Interface may be spoofed by an attacker and this may lead to information disclosure by PLC. Consider using a standard authentication mechanism to identify the destination process.;Modbus RTU on RS-485 has no built-in authentication mechanism and spoofing is feasible from the same bus segment. Evidence for Threat Actor classification is protocol-level OT knowledge requirement with no need for plant-specific sabotage tradecraft, which maps to Cybercriminal as the minimum actor. Thrill Seeker and Hacktivist are excluded because reliable frame crafting on an OT serial bus requires more than opportunistic internet exploit use, and Nation-State Actor is not required because there is no custom multi-stage intrusion or signed supply-chain abuse in this path. Whether the RS-485 module is installed and physically reachable is still unknown, so treatment and approval remain unresolved until deployment evidence confirms scope.;Generated;T0802;TID-118;CWE-287;CVSS:4.0/AV:A/AC:L/AT:N/PR:N/UI:N/VC:N/VI:L/VA:N/SC:N/SI:N/SA:N;"4,3";Medium;Medium;Medium;Cybercriminal;Mitigation;Product Security
  38;Data Flow MCU to CFG over Modbus RTU (RS-232) Is Potentially Interrupted;Denial Of Service;<Device_Name>;MCU to CFG over Modbus RTU (RS-232);Low;Mitigated;;"An external agent interrupts data flowing across a trust boundary in either direction.";"This RS-232 interruption affects a local maintenance session more than the primary control function. Evidence for Threat Actor classification is direct cable proximity, maintenance-port handling, and AV:P local physical access constraints, which makes Insider Threat the minimum actor. Cybercriminal is not the minimum label because no remote foothold or IT-to-OT pivot is required by the modeled path, and Nation-State Actor is unnecessary because the scenario does not require advanced custom tradecraft. Baseline mitigation includes physical port isolation and cable shielding, and residual risk remains Low with closure via design-review sign-off.";Generated;T0814;TID-118;CWE-693;CVSS:4.0/AV:P/AC:L/AT:N/PR:N/UI:N/VC:N/VI:N/VA:L/SC:N/SI:N/SA:L;"2,4";Low;Low;Low;Insider Threat;Mitigation;Lead Security
  72;Elevation Using Impersonation;Elevation Of Privilege;<Device_Name>;Operator to MCU over Switches (GPIO);Low;Not Applicable;;"MCU may be able to impersonate the context of Operator in order to gain additional privilege.";"The dry-contact GPIO path has no machine-to-machine trust boundary the MCU can impersonate and no EMB3D device property enables software privilege escalation on this passive signal path. Evidence for Threat Actor classification is that only local operator-side interaction could influence the interface, which maps to Insider Threat as the minimum actor under AV:P style access constraints. Lower internet-first actors are excluded because there is no remotely reachable identity boundary, and higher strategic actors are unnecessary because no complex tooling or covert persistence is needed. Treatment remains Avoidance by keeping the interface free of machine-authenticated trust assumptions.";Generated;N/A;N/A;N/A;CVSS:4.0/AV:P/AC:L/AT:N/PR:N/UI:N/VC:N/VI:N/VA:N/SC:N/SI:N/SA:N;"0,0";None;Info;Info;Insider Threat;Avoidance;Not Required
  ```
