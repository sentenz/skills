...
| Trojanized engineering software, malicious firmware, or a tainted vendor update used for covert pre-positioning or sabotage | `Nation-State Actor` | Use when the supplier-channel compromise requires sustained stealth, custom tooling, or mission-specific OT manipulation. Example: a trojanized engineering software update that quietly establishes long-term access to safety controllers. |
| Compromised vendor tooling, update service, or MSP remote-management channel reused for broad extortion or monetized access | `Cybercriminal` | Use when the supplier-channel compromise is primarily a scalable access mechanism for fraud, ransomware, or other financially motivated disruption. Example: a compromised remote management platform reused to deploy ransomware across multiple customer sites. |
