...
- For an applied control, lead with product-specific implementation evidence and state the validated control effect. Framework alignment is optional: identify an exact ATT&CK mitigation, CWE mitigation strategy, or EMB3D MID only when it materially explains the control rationale, and do not invent framework provenance for an independently evidenced product control. Do not count the same product control as multiple independent controls merely because several frameworks recommend it.
