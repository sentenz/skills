...
- Reject deprecated and `Prohibited` entries. Accept `Allowed` entries only when affirmative product evidence supports the root cause.
- Prefer `Base` or `Variant` entries when supported, but do not force a narrower mapping beyond the available evidence.
- Retain an `Allowed-with-Review` or `Discouraged` entry only when no better-supported `Allowed` entry exists and record the required `CWE mapping rationale:` in `Justification`.
- A populated `CWE ID` requires affirmative product, architecture, design, implementation, configuration, test, or verified behavioral evidence that establishes the weakness itself. STRIDE category, ATT&CK technique, EMB3D threat, attack vector, attacker capability, exploitation prerequisite, technical impact, CVSS metric, and recommended mitigation data may nominate CWE candidates but must not independently substantiate a CWE mapping.
- For every populated CWE, record exactly one `CWE root-cause evidence: CWE-NNN — <evidence>` clause in `Justification`. The evidence must explain how the assessed product satisfies the selected weakness definition rather than restating the threat outcome.
- When multiple CWEs are mapped, provide separately sufficient root-cause evidence for every ID. Do not stack ancestors, descendants, alternative hypotheses, or merely plausible causes of the same outcome. Use CWE relationships to refine candidate selection, not to inflate the mapping set.
- Use `N/A` when a finalized row describes adversary behavior, a physical threat path, or an impact and the analysis concludes that no underlying product weakness can be identified. Leave `CWE ID` blank and use `Needs Investigation` when missing evidence prevents a defensible root-cause decision.
- Treat `potential_mitigations` as candidate security treatments, not requirements or implementation evidence. Preserve the source mitigation's `mitigation_id` when present, lifecycle `phases`, `strategy`, `effectiveness`, `effectiveness_notes`, and description while evaluating applicability.
- Prefer root-cause treatments applied in Requirements, Architecture and Design, or Implementation when feasible. Later lifecycle controls, monitoring, operational restrictions, and defense-in-depth measures may complement but must not be represented as eliminating the weakness unless the product evidence demonstrates that result.
- Translate each selected CWE candidate mitigation into a product-specific, normative, testable engineering control. Record its applicability rationale and verification method or evidence before using it to justify `State = Mitigated` or to reduce residual risk.
...
- Maintain traceability as `threat → evidenced CWE → candidate mitigation → applicability decision → engineering control → verification evidence → residual risk`. A source-listed mitigation, an assigned CWE, or an implemented-looking recommendation alone does not prove control implementation, effectiveness, or residual-risk reduction.
